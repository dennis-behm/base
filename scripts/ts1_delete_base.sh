...
# Pipeline Workspace
export PIPELINE_WORKSPACE=/u/dbehm/wazi-deploy/workspace/ts1_delete_base
...
if [ $rc -eq 0 ]; then
    wazideploy-deploy.sh -w $application/$branchName/${buildImplementation}build_$timestamp -e EOLEB7-$application-Integration.yaml -l deploy-logs/evidences/evidence.yaml -i ${application}-rel-1.0.0-${timestamp}_delete.tar
    rc=$?
fi